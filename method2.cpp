...
//////////////////////////////////////////////
//NOTE--HERE IM RESIZING THE CROP-AND-WRAP IMAGE NOT THE WHOLE IMAGE---THANK YOU
...
//here my crop and after wrap image size is 327x777.
...
void* mybsfunc (void* arg){
   ...
   ///METHOD--1
//   int *noofframetodrop_ptr = (int*) arg; //taking argument given to us and putting it in noofframetodrop pointer
 //  int mynoofframetodrop=*noofframetodrop_ptr;//putting that value in our variable--mynoofframetodrop

	

   ///METHOD--2--------Here i m extracting x and y coordinate from input argument--for resize
   string *resize_ptr = (string*) arg; //taking argument given to us and putting it in resize pointer
   string mynewresize=*resize_ptr;//putting that value in our variable--mynewresize
   int mynewresizewidth;
   int mynewresizeheight;
   ...
   //string line = "GeeksForGeeks is a must try";
    ...
    stringstream check1(mynewresize);
    ...
    for(unsigned int i = 0; i < tokens.size(); i++){
    	if(i==0){
    	  mynewresizewidth=stoi(tokens[i]);
    	}else if(i==1){
    	  mynewresizeheight=stoi(tokens[i]);
    	}  
   }
    
    Size mynewsize(mynewresizewidth,mynewresizeheight);//I will use this size for resize--
    float mynewtoold_widthratio=((float)mynewresizewidth)/377; //because after crop and wrap our width is 377(fixed see at top)
    float mynewtoold_heightratio=((float)mynewresizeheight)/777; //because after crop and wrap our height is 777(fixed see at top)
    float mynewtoold_arearatio=mynewtoold_widthratio*mynewtoold_heightratio; //
    float mynewtoold_diagonalratio = (float)sqrt(pow((float)mynewresizewidth, 2) + pow((float)mynewresizeheight, 2))/ 863.63;//863.63 is digonal size of 377x777 
    float mynewtoold_aspectratio=(((float)mynewresizewidth/(float)mynewresizeheight))/0.485199;//aspect ration is width-to-height ratio
   
   //cout<<mynewtoold_widthratio<<endl;
   //cout<<mynewtoold_heightratio<<endl;
   //cout<<mynewtoold_arearatio<<endl;
   //cout<<mynewtoold_diagonalratio<<endl;
   //cout<<mynewtoold_aspectratio<<endl;
...
/////////////////////////////////////////////////////////////////////////////////
    ...
    //FOR METHOD---2---resizing the imagess
    resize(myimgFrame1,myimgFrame1,mynewsize);
    resize(myimgFrame2,myimgFrame2,mynewsize);
    
    
    
    
    //this is for part c of assignment-  METHOD--3-------------
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    while (mycapVideo.isOpened() && CheckEscKey != 27) {
		...
		//METHOD--3 BOUNDARY CONDITIONS
            if (mypossibleBlob.mycurrentBoundingRect.area() > 500*mynewtoold_arearatio &&
                mypossibleBlob.mydblCurrentAspectRatio >= 0.2*mynewtoold_aspectratio &&
                mypossibleBlob.mydblCurrentAspectRatio <= 1.25*mynewtoold_aspectratio &&
                mypossibleBlob.mycurrentBoundingRect.width > 20*mynewtoold_widthratio &&
                mypossibleBlob.mycurrentBoundingRect.height > 20*mynewtoold_heightratio &&
                mypossibleBlob.mydblCurrentDiagonalSize > 30.0*mynewtoold_diagonalratio &&
                (contourArea(mypossibleBlob.mycurrentContour) / (double)mypossibleBlob.mycurrentBoundingRect.area()) > 0.40*mynewtoold_arearatio) {
            ...
            if (mypossibleBlob.mycurrentBoundingRect.area() > 500*mynewtoold_arearatio &&
                ...
                &&//----changes for part -3///
                mypossibleBlob.mycurrentBoundingRect.y > 150*mynewtoold_heightratio &&
                mypossibleBlob.mycurrentBoundingRect.y < 600*mynewtoold_heightratio //if our contour is abobe this y then only we will count this--in static
...
//here we will calculate static queue density--------------------------------
	  ...
	  int RealpreviousNoOfBlobs=RealcurrentNoOfBlobs;//for staitcqueque-------
         ...
         if(frameCount>0){//Method-1-METHDO-2--No Problem //METHOD-3 FOR ALL frames  //for METHOD--4//for frame-thread analysis and process
         		 ...
         		 // y only---so that when vehicle just enter and just escaped does not affect our static
                       if( RealpreviousNoOfBlobs> RealcurrentNoOfBlobs){//because density can't be negative
                        ...
                        if(StaticqueueDensity>15){//static queue density cannot be more than 15(observation)
                        	StaticqueueDensity=15;
                        
                        }
                        
                        
                        csv << to_string(frameCount) << to_string(StaticqueueDensity) << to_string(currentNoOfBlobs) <<endrow;
        ...
        RealcurrentFrameBlobs.clear();//for static queue
...
//new addition
	...
	//that is why i have excluded last frmae
	   ...
	   //for--- METHOD--2--resizing the next image---
	   resize(myimgFrame2,myimgFrame2,mynewsize);
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//NO HARM IN UNCOMMENTING THIS IN METHOD--2
}
...
//////////////METHOD--1 --Argument is Number of frame to be dropped
	string myresolution = argv[1];//convert string argument into integer value
	...
	pthread_create(&tid, &attr, mybsfunc, &myresolution);//just making another thread and its function is our mybsfunc
