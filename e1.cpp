...
//here we will calculate static queue density--------------------------------
	  ...
	  int RealpreviousNoOfBlobs=RealcurrentNoOfBlobs;//for staitcqueque-------
         ...
         if(frameCount%5==0){//Method-1-METHDO-2--No Problem //METHOD-3 FOR ALL frames  //for METHOD--4//for frame-thread analysis and process
         		 ...
         		 // y only---so that when vehicle just enter and just escaped does not affect our static
                       if( RealpreviousNoOfBlobs> RealcurrentNoOfBlobs){//because density can't be negative
                        ...
                        if(StaticqueueDensity<0  ){//it cant be negative and maximally it could be 17
                        StaticqueueDensity=0;
                        }
                        if(StaticqueueDensity>17){
                        	StaticqueueDensity=17;
                        
                        }
                        
                       cout<<  to_string(frameCount)+","+ to_string(StaticqueueDensity)+","+to_string(currentNoOfBlobs)<<endl;
