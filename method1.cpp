...
//////////////////////////////////////////
//WHNE NUMBER OF FRAMEDROP IS --5 it gave us the best result thankyou
...
//  int mynoofsplit =(int)arg_struct->noofsplit ; // extracting information from our  structure
  ...
  // int mymycouncounter=(int) arg_struct->mycouncounter;
   ...
   VideoCapture mycapVideo;
    ...
    mycapVideo.open("/home/user/Downloads/2018CS10333_2019CS50430_ass1_part3/trafficvideo.mp4");
    ...
    while (mycapVideo.isOpened() && CheckEscKey != 27) {
		...
		//METHOD--3 BOUNDARY CONDITIONS
            ...
            if (mypossibleBlob.mycurrentBoundingRect.area() > 500 &&
                ...
                &&//----changes for part -3///
                mypossibleBlob.mycurrentBoundingRect.y > 150&&
...
//here we will calculate static queue density--------------------------------
	  ...
	  int RealpreviousNoOfBlobs=RealcurrentNoOfBlobs;//for staitcqueque-------
         ...
         if(frameCount%mynoofframetodrop==0){//METHOD-3 FOR ALL frames  //for METHOD--4//for frame-thread analysis and process
...
//////////////METHOD--1 --Argument is Number of frame to be dropped
	int noofframedrop = stoi(argv[1]);//convert string argument into integer value
	pthread_t tid;
	...
	pthread_create(&tid, &attr, mybsfunc, &noofframedrop);//just making another thread and its function is our mybsfunc
