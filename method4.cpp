...
/////////////////////////////////////////////////////////////////////////////////
    ...
    float heightpersplit= myimgheight/mynoofsplit;
...
//here we will calculate static queue density--------------------------------
	  ...
	  int RealpreviousNoOfBlobs=RealcurrentNoOfBlobs;//for staitcqueque-------
         ...
         if(frameCount%mynoofsplit==mysplitno){//for METHOD--4//for frame-thread analysis and process
         		 ...
         		 // y only---so that when vehicle just enter and just escaped does not affect our static
                       if( RealpreviousNoOfBlobs> RealcurrentNoOfBlobs){//because density can't be negative
                        ...
                        if(StaticqueueDensity>15){//static queue density cannot be more than 17(observation)
                    ...
                    /*    
                       NOTE--WHEN U USE THIS COMMAND FOR PRINTING---
                       -----U GET TO KNOW --HOW DIFF-DIFF THREADS WORKING ON OUR DIFF-DIFF FRAMES
                       
                       ---"mysplitno"-HERE WILL TELL U WHICH THREAD IS WORKED UPON OUR FRAME===
                       
                       ----NOTE-ALSO NOTE SOMETIMES THREADS ARE NOT ALWAYS IN SEQUENCE --SOMETIMES
                       ---SOME OTHER THREAD MAY COME BEFORE SOME OTHER THREAD WHICH WAS USED LATER BEFORE--
                       ---THEY ARE NOT ALWAYS IN SYCRONOUS TO EACH OTHER--(i.e--NO SEQUENCE IN OPERATION
                       OF VARIOUS THREADS)
                       
                       --BUT WHEN NOOFTHREAD=2---THERE IS NO SYCHRONOUS ISSUE BETWEEN THREADS-----
                    
         		cout<<  to_string(mysplitno)+"," +to_string(frameCount)+","+ to_string(StaticqueueDensity)+","+to_string(currentNoOfBlobs)<<endl;
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	pthread_exit(0);
	//NO PROBLEM IN UNCOMMENTING IT OUT
}
...
int main(int argc, char **argv) {//taking input from user
	if (argc < 2) {
		printf("METHOD-3 Give argument --numberofpthreads/no of split of our frameUsage");
